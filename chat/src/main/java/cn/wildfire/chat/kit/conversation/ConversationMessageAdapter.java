...
public class ConversationMessageAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        if (sendLayoutRes != null) {
            sendResId =FileUtils.getId(sendLayoutRes.resId());
        } else if (layoutRes != null) {
            sendResId = FileUtils.getId(layoutRes.resId());
        }

        if (receiveLayoutRes != null) {
            receiveResId = FileUtils.getId(receiveLayoutRes.resId());
        } else if (layoutRes != null) {
            receiveResId =  FileUtils.getId(layoutRes.resId());
        }
        ...
        if (SimpleNotificationMessageContentViewHolder.class.isAssignableFrom(viewHolderClazz)) {
            ...
            viewStub.setLayoutResource( FileUtils.getId(layoutRes.resId()));
        } else {
            if (direction == 0) {
